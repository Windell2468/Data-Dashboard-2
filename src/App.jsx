...
function App() {
  // The state for storing fetched Disney Characters
  const [characters, setCharacters] = useState([])
  // The State to manage loading status during API fetch
  const [isLoading, setIsLoading] = useState(true)
  // The text input for search disney character name
  const [search, setSearch] = useState("")
  // Choose film in the dropdown filter
  ...
  const filteredCharacters = characters
    .filter((char) => char.name.toLowerCase().includes(search.trim().toLowerCase()))
    .filter((char) => {
        ...
        {isLoading ? (
          ...
          <div>
            ...
            <section className="summary">
              ...
              <div className="stat">
                <strong>Avg. films per character:</strong> {avgFilmsPerChar.toFixed(2)}
              </div>
            </section>
            ...
            <section className="controls">
              ...
              <label>
                ...
                <select value={chooseFilm} onChange={(e) => setChooseFilm(e.target.value)} >
            ...
            <section className="results">
              <p>
                Showing {Math.min(filteredCharacters.length, 10)} of {filteredCharacters.length} matching characters
              </p>
              <ul>
                    ...
                    <Link to={`/character/${char._id}`} className="char-link">
                      ...
                      <div className="char-feature">
                        {char.films && char.films.length > 0 ? (
                          <p><strong>Films:</strong> {char.films.join(", ")}</p>
                        ) : (
                          <p><strong>Films:</strong> None</p>
                        )}
                        {char.tvShows && char.tvShows.length > 0 ? (
                          <p><strong>TV Shows:</strong> {char.tvShows.join(", ")}</p>
                        ) : (
                          <p><strong>TV Shows:</strong> None</p>
                        )}
                        {char.videoGames && char.videoGames.length > 0 ? (
                          <p><strong>Video Games:</strong> {char.videoGames.join(", ")}</p>
                        ) : (
                          <p><strong>Video Games:</strong> None</p>
                        )}
                        {char.allies && char.allies.length > 0 ? (
                          <p><strong>Allies:</strong> {char.allies.join(", ")}</p> 
                        ) : (
                          <p><strong>Allies:</strong> None</p> 
                        )}
                        {char.enemies && char.enemies.length > 0 ? (
                          <p><strong>Enemies:</strong> {char.enemies.join(", ")}</p>
                        ) : (
                          <p><strong>Enemies:</strong> None</p>
                        )}
                      </div>
                    </Link>
                  </li>
            ...
            {filteredCharacters.length === 0 && (
              <p>
                No Disney Characters found matching "{search}" and film "{chooseFilm === "All" ? "Any" : chooseFilm}".
              </p>
            )}
          </div>
        )}
